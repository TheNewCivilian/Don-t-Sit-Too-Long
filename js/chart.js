nv.addGraph(function() {
  ...
  chart.x2Axis.showMaxMin(false).tickFormat(function(d) {
    return d3.time.format("%x")(new Date(d))
  })

  chart.yAxis.axisLabel("Points").tickFormat(d3.format(",r"))

  chart.y2Axis.tickFormat(d3.format(",r"))
...
function data() {
  ...
  for (var i = 0; i < logjson.length; i++) {
    ...
    timestamp = new Date(
      parseInt(item.time.year),
      parseInt(item.time.month) - 1,
      parseInt(item.time.day),
      parseInt(item.time.hour),
      parseInt(item.time.minute),
      0,
      0
    )
